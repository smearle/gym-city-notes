...
\tikzset{hidden/.append style={xshift=35}}
\tikzset{hidden/.append style={yshift=-80}}
\node[hidden, draw=none, fill opacity=0, text opacity =1, font=\normalsize, yshift=42] at (-3,5.5) {
    intra-column sharing:\\
    $\forall i, j, j' \quad f_{ij} = f_{ij'} $\\\\
    inter-column sharing:\\
    $\forall i, i', j, j' \quad f_{ij} = f_{i'j'} $\\
};
\end{tikzpicture}
